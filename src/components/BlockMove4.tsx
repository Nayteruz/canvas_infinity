...
const BlockMove = observer(() => {
    ...
    useEffect(() => {
        ...
        layerRef.current.onwheel = (e: WheelEvent) => {
            e.preventDefault();
            // коэффициент скорола
            const speedFactorScroll = (e.deltaMode > 100 ? 0.1 : e.deltaMode ? 1 : 0.005);
            const pinchScroll = -e.deltaY * speedFactorScroll;
            // коэффициент масштабирования
            const speedFactorScale = (e.deltaMode === 1 ? 0.05 : e.deltaMode ? 1 : 0.002);
            ...
            if (e.shiftKey) {
                ...
                // startTtansform(e.clientX - xs * scale, e.clientY - ys * scale, scale);
                // setTransform
            } else {
                setPoint((prev) => {
    ...
    return (
        ...
        >
            <div
                className="container"
                style={{ backgroundPosition: `${transform.x * transform.s}px ${transform.y * transform.s}px` }}
            >
                <div
